...
while (my $line=<FILELIST>){
  ...
  while(<TESTFILE>){
    ...
    if (($_ =~ /^\s*#`ifdef\s+(\w+)/) || ($_ =~ /^\s*\/\*#`ifdef\s+(\w+)/) || ($_ =~ /^\s*\/\*#`ifdef\s+(\w+)\s*\*\//)){
    ...
    if (($_ =~ /^\s*#`endif\s+(\w+)/) || ($_ =~ /^\s*\/\*#`endif\s+(\w+)/) || ($_ =~ /^\s*\/\*#`endif\s+(\w+)\s*\*\//)){
      if($cnt == 0){
        $print_or_not_print = 1;
      }
      else{
	...
	if (($_ =~ /^\s*#`ifndef\s+(\w+)/) || ($_ =~ /^\s*\/\*#`ifndef\s+(\w+)/) || ($_ =~ /^\s*\/\*#`ifndef\s+(\w+)\s*\*\//)){
      if(exists $hash_config{$1} == 0){
          if($cnt == 0){
            $print_or_not_print = 1;
          }
    ...
    if (($_ =~ /^\s*#`endnif\s+(\w+)/) || ($_ =~ /^\s*\/\*#`endnif\s+(\w+)/) || ($_ =~ /^\s*\/\*#`endnif\s+(\w+)\s*\*\//)){
      if($cnt == 0){
        $print_or_not_print = 1;
      }
      else{
